...
def get_sequence_chain(structure1):

    """
    Given a structure returns its sequence
    """
    aminoacids = [x.get_resname() for x in structure1.get_residues() if x.has_id("CA")]
...
def Alignsequence(sequence1, sequence2):
    """
...
for file1 in files_list:
    structures[file1] = GetStructures(file1)
...
for file1, structure in structures2.items():
        ...
        if file2 not in all_files:
            ...
            if file1 != file2:
                ...
                for chain in chains1:
                    sequence1 = get_sequence_chain(chain)
                #     #chains_copy.remove(chain)
                    for chain2 in chains2:
                        sequence2 = get_sequence_chain(chain2)
                        Alignment = Alignsequence(sequence1, sequence2)
                        scores[file1 + file2].append(Alignment[0][2]/len(list(chain.get_residues())))
               
                same_chains = list(filter(lambda x: x[1] > 0.95, enumerate(scores[file1 + file2])))
                ...
                if len(same_chains) == 2 or len(same_chains) == 4:
                    ...
                    print(distances1, distances2, file1 + file2)
